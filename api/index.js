...
app.put('/api/products/:id', async (req, res) => {
    try {
        ...
        // Then insert as new (to avoid duplicate issues)
        const { data, error } = await supabase
            .from('products')
            .insert([{
                name, category,
...
app.put('/api/cart/:userId', async (req, res) => {
    try {
        ...
        if (!existingCart) {
            await supabase.from('carts').insert([{ user_id: req.params.userId, items: items }]);
        } else {
            await supabase.from('carts').update({ items, updated_at: new Date() }).eq('user_id', req.params.userId);
        }
...
app.get('/api/wishlist/:userId', async (req, res) => {
    try {
        ...
        if (!data) {
            await supabase.from('wishlists').insert([{ user_id: req.params.userId, product_ids: [] }]);
            res.json([]);
        } else {
...
app.put('/api/wishlist/:userId', async (req, res) => {
    try {
        ...
        if (!existingWishlist) {
            await supabase.from('wishlists').insert([{ user_id: req.params.userId, product_ids: productIds }]);
        } else {
            await supabase.from('wishlists').update({ product_ids: productIds, updated_at: new Date() }).eq('user_id', req.params.userId);
        }
...
app.post('/api/orders', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            .from('orders')
            .insert([{
                order_id: orderId, user_id: userId, items, customer, total,
                delivery_charge: deliveryCharge, payment_method: paymentMethod,
                sender_number: senderNumber, transaction_id: transactionId,
...
app.put('/api/orders/:orderId', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            .from('orders')
            .update({ order_status: orderStatus, payment_status: paymentStatus, tracking })
